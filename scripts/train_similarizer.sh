...
bs=${4:-16}
emb=${5:-"sbert"}
MM_INV_PROJECTOR_TYPE="similarizer_mlp2x"
DENOISED_EMBEDDING_TYPE="lgd_sbert"

similarity_loss_weight=1    # cora(1)    Pubmed(1.4)    reddit(0.8)    arxiv(1)
...
prefix=reconglm_similarizer-vicuna-7b-${emb}-${use_hop}-${sample_size}-${projector_type}-projector-${DENOISED_EMBEDDING_TYPE}-${MM_INV_PROJECTOR_TYPE}-w${similarity_loss_weight}
...
--version ${mode} \
--cache_dir  ./checkpoints/reconglm_similarizer \
--pretrained_embedding_type ${emb} \
--mm_use_graph_start_end False \
--mm_use_graph_patch_token False \
--output_dir  ./checkpoints/reconglm_similarizer/${dataset}/${prefix}_${task} \
--num_train_epochs 1 \
